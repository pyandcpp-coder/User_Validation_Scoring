...
# core/scoring_engine.py (Complete Production Version)
...
class ScoringEngine:
    def __init__(self):
        ...
        try:
            ...
            self._initialize_database()
            print(f"ScoringEngine: Successfully connected to PostgreSQL at {db_host}.")
        except psycopg2.OperationalError as e:
    ...
    def add_qualitative_post_points(self, user_id: str, text_content: str, image_path: Optional[str], originality_distance: float) -> float:
        """Calculates and adds qualitative post points. Returns points awarded."""
        self._ensure_user_exists(user_id)
        with self.conn.cursor() as cur:
            cur.execute("SELECT points_from_posts FROM user_scores WHERE user_id = %s;", (user_id,))
            current_post_points = cur.fetchone()[0]
        ...
        if round(current_post_points, 2) >= config.MAX_MONTHLY_POST_POINTS:
            print(f"User {user_id} reached monthly post point limit.")
            return 0.0
    ...
    def add_like_points(self, user_id: str) -> float:
        """Adds points for a like and returns points awarded."""
        return self._add_simple_points(user_id, 'likes', config.POINTS_PER_LIKE, config.MAX_MONTHLY_LIKE_POINTS)

    def add_comment_points(self, user_id: str) -> float:
        """Adds points for a comment and returns points awarded."""
        return self._add_simple_points(user_id, 'comments', config.POINTS_PER_COMMENT, config.MAX_MONTHLY_COMMENT_POINTS)

    def add_referral_points(self, user_id: str) -> float:
        """Adds points for a referral and returns points awarded."""
        return self._add_simple_points(user_id, 'referrals', config.POINTS_PER_REFERRAL, config.MAX_MONTHLY_REFERRAL_POINTS)

    def add_tipping_points(self, user_id: str) -> float:
        """Adds points for tipping and returns points awarded."""
        return self._add_simple_points(user_id, 'tipping', config.POINTS_FOR_TIPPING, config.MAX_MONTHLY_TIPPING_POINTS)

    def _add_simple_points(self, user_id: str, column_name: str, points_to_add: float, max_points: float) -> float:
        """A generic and robust helper to add points for simple interactions."""
        self._ensure_user_exists(user_id)
        with self.conn.cursor() as cur:
            # First, get the current points for the user
            cur.execute(f"SELECT points_from_{column_name} FROM user_scores WHERE user_id = %s;", (user_id,))
            record = cur.fetchone()

            current_points = record[0] if record else 0.0
            
            if round(current_points, 2) >= max_points:
                print(f"User {user_id} has reached the monthly '{column_name}' point limit.")
                return 0.0
            cur.execute(f"""
                UPDATE user_scores
                SET points_from_{column_name} = LEAST(%s, points_from_{column_name} + %s)
                WHERE user_id = %s;
            """, (max_points, points_to_add, user_id))
            self.conn.commit()
            
            print(f"Awarded {points_to_add:.2f} points for '{column_name}' to user {user_id}.")
            return points_to_add

    def get_final_score(self, user_id: str) -> float:
        """Fetches all points and calculates the final 0-100 score."""
        self._ensure_user_exists(user_id)
        with self.conn.cursor() as cur:
            cur.execute("SELECT points_from_posts, points_from_likes, points_from_comments, points_from_referrals, points_from_tipping FROM user_scores WHERE user_id = %s;", (user_id,))
            record = cur.fetchone()
            if not record: return 0.0
            
            total_monthly_points = sum(record)
            normalized_score = (total_monthly_points / config.TOTAL_POSSIBLE_MONTHLY_POINTS) * 100
